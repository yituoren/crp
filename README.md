...
权限在服务端校验；跟队/站点/直播员身份来自主办在「排班」里的分配，按赛段生效。直播员跨整个赛段跟进所有数据，只读。
...
- 跟队和站点只能在进行中的赛段记录；主办不受此限（用于事后补录）。
- 每支队伍必须按环节顺序打卡：前面的必经环节（Starting Line、任务点、绕道、路障、联合、洗牌、陷阱、中继站）没有完成，后面的环节不能记录。快进、减速带、回转点、让路点、对抗只有部分队伍会经过，不作为前置条件；成功快进的队伍可直接到中继站签到。
- 撤销记录只能从最后一个有记录的环节开始往前撤。主办在「修改」弹窗里的手工修正不受顺序限制。

## 线索卡模板与配色

`docs/templates/三折页线索模板.docx` 是线索卡的打印模板，`docs/templates/cards/` 里是各类卡片的缩略图。页面上环节类型标签的颜色取自这些卡片的主色：起跑线 #C9DA4D、路线 #009DDA、任务 #33CBCC、绕道 #EAAA16、路障 #AC0000、快进 #27CE62、联合 #EB661C、对抗 #664193、减速带 #FFCC01。模板里没有的类型（回转点、让路点、陷阱、中继站、洗牌）另选了不冲突的颜色，定义在 `web/src/styles.css`。
