...
- 金额精确到分，页面按元显示两位小数。
- 队伍默认没有初始货币（可在「赛事设置」里改）。经费按赛段发放：主办在「赛段信息」里给赛段填「每队经费」，点「开始赛段」时系统自动给每支存活队伍加上这笔钱并写入货币流水，只发一次。
...
- 每支队伍必须按环节顺序打卡：前面的必经环节（起跑线、任务点、绕道、路障、联合、陷阱、中继站）没有完成，后面的环节不能记录。快进、减速带、回转点、让路点、对抗只有部分队伍会经过，不作为前置条件；成功快进的队伍可直接到中继站签到。
