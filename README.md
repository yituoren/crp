...
权限在服务端校验；跟队/站点/直播员身份来自主办在「排班」里的分配，按赛段生效。站点不记录时间，负责罚时、补时、经费和附件；直播员跨整个赛段跟进所有数据，只读。
...
- 每支队伍必须按环节顺序打卡：前面的必经环节（Starting Line、任务点、绕道、路障、联合、洗牌、陷阱、中继站）没有完成，后面的环节不能记录。快进（FF）、减速带、回转点、让路点、对抗（FO）只有部分队伍会经过，不作为前置条件；成功快进的队伍可直接到中继站签到。
