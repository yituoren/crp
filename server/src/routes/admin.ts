...
adminRoutes.get('/admin/settings', (c) =>
  c.json({ eventName: getSetting('event_name'), initialCurrency: Number(getSetting('initial_currency', '1000')), hosts: getSetting('hosts') }),
);
adminRoutes.put('/admin/settings', async (c) => {
  const b = await body(c);
  if (b.eventName !== undefined) setSetting('event_name', str(b.eventName, 50) || '城市飞奔');
  if (b.initialCurrency !== undefined) setSetting('initial_currency', String(int(b.initialCurrency, 1000)));
