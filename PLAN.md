...
| FF | 快进 Fast Forward | 全赛程仅一队可用，成功直达终点 | 谁尝试、是否成功 |
...
2026-09-14 按贴吧直播贴与百度百科的赛事记录补充：CRP 实际使用的术语还有 让路点、回转点（双回转）、减速带、联合、对抗/PK、捷径、突然死亡、回忆任务、中继站，以及任务奖励经费、召车券、让行卡等道具。系统已加入 UT 回转点、YD 让路点、SB 减速带、FO 对抗（Face Off）、Shuffle 洗牌五种类型；快进缩写为 FF，并为每个环节增加「是否需要站点人员」「记录方式（不记录 / 只记一次 / 到达+完成）」两项设置，按类型给默认值。道具（召车券、让行卡）暂用货币流水的备注或环节备注记录，后续可单独建表。
...
| 层 | 选型 | 理由 |
|---|---|---|
| 前端 | Vue 3 + Vite + TypeScript，手写 CSS 沿用原型风格 | 移动端优先，幕后全在手机上操作 |
| 后端 | Node.js + Hono（或 Fastify）+ TypeScript | 轻、单进程够用；和前端同语言 |
| 数据库 | SQLite（Node 内置 `node:sqlite`，无需原生编译） | 单机、零运维、一场比赛几万条记录绰绰有余；备份就是复制一个文件 |
...
| 部署 | Vultr 东京 VPS（用户已有；1C1G 起步够用），Node + systemd 直接监听 80，IP + HTTP 访问；Docker 为可选 | 境内手机访问延迟约 80–150ms；后续如需 PWA/离线队列再加域名 + Caddy HTTPS |
...
> 进度（2026-09-09）：阶段 1–3 已实现并本地验证；阶段 4 完成移动端适配与请求重试（PWA/离线队列因无 HTTPS 推迟）；阶段 5 完成部署脚本（`deploy/`）、备份脚本与操作手册，待在 Vultr 上实际上线与多手机演练。
> 实际选型与计划的差异：前端用手写 CSS（沿用原型风格）而非 Tailwind；数据库用 Node 内置 `node:sqlite` 而非 better-sqlite3 + Drizzle，省去原生编译；部署用 systemd 而非 Docker（Dockerfile 保留为可选）。
...
2. ~~**单场还是多场**~~ 已定：只服务 BJ20 这一场（赛事名、初始货币可在后台改；换比赛用「重置比赛数据」）。
3. ~~**选手是否登录**~~ 已定：暂不做选手页面。
4. ~~**附件规模**~~ 已定：本地磁盘，单文件 10MB 上限，图片/PDF。
5. ~~**名次规则**~~ 已定：按签到时间 + 罚时自动排名，主办可手工改；绕道/路障次数不做硬性校验。
6. ~~**UI 风格**~~ 已定：沿用原型蓝色卡片风。
