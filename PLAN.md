...
2026-09-14 按贴吧直播贴与百度百科的赛事记录补充：CRP 实际使用的术语还有 让路点、回转点（双回转）、减速带、联合、对抗/PK、捷径、突然死亡、回忆任务、中继站，以及任务奖励经费、召车券、让行卡等道具。系统已加入 UT 回转点、YD 让路点、SB 减速带、PK 对抗、Shuffle 洗牌五种类型，并为每个环节增加「是否需要站点人员」「记录方式（不记录 / 只记一次 / 到达+完成）」两项设置，按类型给默认值。道具（召车券、让行卡）暂用货币流水的备注或环节备注记录，后续可单独建表。
